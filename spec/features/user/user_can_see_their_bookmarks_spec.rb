...
describe 'A registered user' do
  it 'can add videos to their bookmarks' do
    ...
    visit '/dashboard'
    within('#bookmarks') do
      expect(page).to have_link(tutorial1.title.to_s, count: 2)
      expect(page).to have_link(tutorial2.title.to_s, count: 3)
      within('#video-1') { expect(page).to have_link(video1.title.to_s) }
      within('#video-2') { expect(page).to have_link(video2.title.to_s) }
      within('#video-3') { expect(page).to have_link(video4.title.to_s) }
      within('#video-4') { expect(page).to have_link(video5.title.to_s) }
      within('#video-5') { expect(page).to have_link(video3.title.to_s) }
      expect(page).to_not have_link(video6.title.to_s)
      expect(page).to_not have_link(video7.title.to_s)
    end
  end
end
